...
def find_bracket(f, args, xmin, xmax, n, x_pre=None, scan_window=10, backsearch=False):
    ...
    y1 = f(x_list[0], *args)
    for idx, i in enumerate(x_list[1:], start=1):
        y2 = f(i, *args)
        if y1 * y2 <= 0.0:
            return True, (x_list[idx - 1], x_list[idx])
    ...
    #     return False, (None, None)
    # else:
    #     return True, (x_list[idx[0][0]], x_list[idx[0][0] + 1])
...
class Section:
    ...
    def find_root(self, V0, omega, phi_pre=None, rho=1.225, mu=1.81e-5, sos=340.0) -> SectionAero:
        ...
        for i in order:
            ...
            if not startfrom90:
                ...
                if phi_max == np.pi / 2.0:
                    backsearch = True
            sucess, bracket = find_bracket(self.residual, args=(oper,), xmin=phi_min, xmax=phi_max, n=50, scan_window=10.0, x_pre=phi_pre, backsearch=backsearch)
            if sucess:
